...
def build_teacher_prompt(
    ...
    instructions = (
        ...
        "For pair=0 also set eta_communication=0.0 and eta_near=0.0. For pair>0, "
        "sensing_power_w equals eta_haps*(1-eta_communication)*maximum_haps_power_w "
        "and must meet minimum_sensing_power_w. Low communication power can make SIC "
        "infeasible, so include robust high-power communication choices and keep "
        "eta_near conservative. Favor freshness while respecting sensing, SIC, secrecy "
        "and long-term queues. Return only JSON with schema_version=1, the exact "
